...
def async_fetch_user_data(self, user_id: str) -> Dict[str, Any]:
    ...
    async def _fetch_data():
        ...
        try:
            ...
            # Cache the data (synchronous cache API)
...
def async_build_prompt(self, user_data: Dict[str, Any], recommendation_type: str) -> Dict[str, Any]:
    ...
    async def _build_prompt():
        ...
        try:
            ...
            # Normalize recommendation type (case-insensitive, fallback to PLACE)
            norm_type = (recommendation_type or "place").lower()
            valid_values = [rt.value for rt in RecommendationType]
            if norm_type not in valid_values:
                norm_type = RecommendationType.PLACE.value
            ...
            if missing_any:
                ...
                # build_fallback_prompt is synchronous
                try:
                    rec_enum = RecommendationType(norm_type)
                except Exception:
                    rec_enum = RecommendationType.PLACE
                prompt = prompt_builder.build_fallback_prompt(rec_enum)
            else:
            ...
            # Cache the prompt (synchronous cache API)
            cache_service.set("prompt", f"{user_data.get('user_id', 'unknown')}_{norm_type}", {
                "prompt": prompt,
                "recommendation_type": norm_type,
                "built_at": time.time()
            })
...
def async_call_llm(self, prompt: str, user_context: Dict[str, Any], recommendation_type: str) -> Dict[str, Any]:
    ...
    async def _call_llm():
        ...
        try:
            ...
            # Call LLM service (supports async path if available, else sync)
            try:
            ...
            # Cache the recommendations (synchronous cache API)
            user_id = user_context.get("user_id", "unknown")
...
def async_cache_results(self, user_id: str, recommendations: List[Dict[str, Any]], recommendation_type: str) -> Dict[str, Any]:
    ...
    async def _cache_results():
        ...
        try:
            ...
            # Store recommendations in cache (prefer async, fallback to sync)
            try:
                ...
                llm_service._store_in_redis(user_id, {
                    "recommendations": recommendations,
                    "recommendation_type": recommendation_type,
                    "generated_at": time.time()
                })
    ...
    async def _generate_recommendations():
        ...
        try:
            ...
            if not force_refresh:
                cached_data = cache_service.get("recommendations", f"{user_id}_{norm_type}")
                if cached_data:
                    logger.info("Using cached recommendations", user_id=user_id, recommendation_type=recommendation_type)
                    # Normalize cached recommendations shape
                    cached_recs = cached_data.get("recommendations", cached_data)
                    if isinstance(cached_recs, dict):
                        ...
                        key = type_to_key.get(norm_type, "places")
                        cached_recs = cached_recs.get(key, [])
                    elif not isinstance(cached_recs, list):
                        cached_recs = []
                    return {
                        "success": True,
                        "user_id": user_id,
            ...
            # Build prompt
            prompt_result = async_build_prompt(user_data, norm_type)
            ...
            # Call LLM
            llm_result = async_call_llm(prompt, user_data, norm_type)
            ...
            # Cache results
            cache_result = async_cache_results(user_id, recommendations, norm_type)
